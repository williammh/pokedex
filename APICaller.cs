...
namespace playground
{
    public class WebRequest
    {
        ...
        public static async Task GetPokemonNameDataAsync(string PokemonName, Action<Dictionary<string, object>> Callback)
        {
            using (var Client = new HttpClient())
            {
                try
                {
                    Client.BaseAddress = new Uri($"http://pokeapi.co/api/v2/pokemon/{PokemonName}");
